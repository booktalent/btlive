...
def generate_contract_pdf(booking: dict, artist: dict, customer: dict, contract: dict) -> bytes:
    ...
    fin_rows = [
        ["Description", "Amount"],
        ["Package Fee", _money(pricing.get("package_fee", 0))],
        ["Add-ons Total", _money(pricing.get("addons_total", 0))],
        ["Coupon Discount", f"- {_money(pricing.get('coupon_discount', 0))}"],
        [f"Platform Fee ({pricing.get('platform_fee', 0) and '5%' or '—'})", _money(pricing.get("platform_fee", 0))],
        ["GST (18%)", _money(pricing.get("gst", 0))],
        ["TOTAL", _money(pricing.get("total", 0))],
        ["Token Paid", _money(booking.get("amount_paid", 0))],
        ["BALANCE DUE", _money(pricing.get("total", 0) - booking.get("amount_paid", 0))],
    ]
    ...
    terms = [
        "1. The Artist agrees to perform as described above on the agreed date and time.",
        "2. The Client agrees to provide stage, sound system, electricity, hospitality and other technical requirements as per the package rider.",
        "3. CANCELLATION POLICY: Cancellations 15+ days prior to the event qualify for a full refund of the advance token. Cancellations 7-14 days prior receive a 50% refund of the advance. Cancellations within 7 days of the event forfeit the entire token amount.",
        "4. ARTIST CANCELLATION: In the event the Artist cancels for any reason, 100% of all amounts paid will be refunded and the Client will receive priority rebooking assistance.",
        "5. The balance amount stated above must be paid in full at least 24 hours before the event start time, either directly to the Artist or through BookTalent's escrow.",
        "6. All disputes shall be resolved through BookTalent's grievance system and are subject to the jurisdiction of Mumbai, India.",
        "7. This contract is auto-generated by BookTalent and is governed by BookTalent's Master Service Agreement (https://booktalent.com/legal).",
        "8. Both parties acknowledge digital signature via the BookTalent platform constitutes acceptance of all terms herein.",
    ]
    for t in terms:
...
def generate_invoice_pdf(booking: dict, artist: dict) -> bytes:
    ...
    elems.append(Paragraph("Tax Invoice", S["title"]))
    ...
    rows = [
        ...
        ["Artist", artist.get("stage_name") or artist.get("first_name", "—")],
    ...
    bill = [
        ["Description", "Amount"],
        ["Performance Package", _money(pricing.get("package_fee", 0))],
        ["Add-ons", _money(pricing.get("addons_total", 0))],
        ["Discount", f"- {_money(pricing.get('coupon_discount', 0))}"],
        ["Platform Fee", _money(pricing.get("platform_fee", 0))],
        ["GST @ 18%", _money(pricing.get("gst", 0))],
        ["TOTAL", _money(pricing.get("total", 0))],
    ]
